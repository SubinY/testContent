...
### 本次需求
- 回退上一次对“垂直顶部对齐”和“预览/导出统一渲染链路”的改动，恢复到上一版可用状态。

### 改动后
- `lib/export.ts` 回退为上一版居中布局样式（撤销顶部对齐改动）。
- `lib/screenshot.ts` 回退为上一版独立绘制截图方案（撤销 iframe 页面态截图链路）。

### 影响评估
- 当前表现已恢复到你确认可用的上一个版本，可在该基线继续迭代。
